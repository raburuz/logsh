...
export const Feed = () => {
  ...
  const loadNextEvents = async () => {
    await event.fetchEvents( project.selectedWorkspaceId );
  }
    ...
    <section className='w-full z-20 mb-8 pt-10'>
      {/* Section header */}
         <div className="flex items-center gap-2 mb-8 text-[11px] font-mono text-muted-foreground/50">
            ...
            <span className="uppercase text-emerald-500/65">{project.project.workspaces.list.filter(wk => wk.id === project.selectedWorkspaceId)[0]?.name}</span>
            ...
            project.selectedWorkspaceId && (
              <>
                ...
                <WKSettings
                  ...
                  workspace={project.project.workspaces.list.filter(wk => wk.id === project.selectedWorkspaceId)[0]}
                />
              </>
            )
          }
      ...
      >
        {
          project.isLoading && [1,2,3,4].map( _ => <div key={_} className="-mt-2 w-20 px-3 h-4 py-1.5 bg-zinc-800 rounded-md animate-pulse" />)
        }
        {
          !project.isLoading && project.project.workspaces.list.length === 0 && (<NoWorkspaces />)
        }
        { 
          project.project.workspaces.list.map((wk, index) => (
            <div key={wk.id}>
              <div
                role="tab"
                aria-selected={project.selectedWorkspaceId === wk.id}
                onClick={ async () => {
                  if(project.selectedWorkspaceId === wk.id) return;
                  await project.selectWorkspaceById(wk.id);
                }}
                className={cn(
                  ...
                  project.selectedWorkspaceId === wk.id
